...
function loadBarangayDeliveries(barangay) {
    ...
    const unsub = onSnapshot(q, (snapshot) => {
        ...
        snapshot.forEach((docSnap) => {
            ...
            // Only show Print button - no duplicate Received button needed
            // Status is already shown in the STATUS column
            const printBtn = document.createElement('button');
            printBtn.innerHTML = '<span class="btn-icon">⎙</span><span class="btn-text">Print</span>';
    ...
    deliveries.forEach((delivery) => {
        ...
        tdAction.innerHTML = `
            <div class="action-cell">
                <button class="print-btn" onclick="printDeliveryReceipt('${delivery.id}')" style="margin-right: 10px;">
...
window.loadResidentsForBarangay = async function(barangayName) {
    ...
    residents.forEach((resident) => {
        ...
        // Debug log to check remarks data
        if (resident.remarks) {
            console.log('Resident with remarks:', resident.name, 'Remarks:', resident.remarks);
        }
        ...
        cells.forEach((c, idx) => {
            ...
            // Add special styling for remarks column (index 11)
            if (idx === 11) {
            ...
            setTimeout(() => {
                try {
                    ...
                    if (btns && searchSection) {
                        ...
                        // Remove any icon-labeled duplicate buttons, keep plain text ones
                        try {
                            const hasIcon = (text) => /[\u{1F300}-\u{1FAFF}\u2600-\u27BF]/u.test(text);
                            btns.querySelectorAll('button').forEach((b) => {
                                const label = (b.textContent || '').trim();
                                if (label && hasIcon(label)) {
                                    b.remove();
                                }
                            });
                        } catch(_) {}
        ...
        try {
            if (window.jQuery && $('#viewResidentsTable').length) {
                ...
                const dt = $('#viewResidentsTable').DataTable({
                    ...
                    searching: false // Disable DataTable search (we have custom search)
                });
        ...
        if (deliveries.length === 0) {
            ...
            if (completedDeliveries.length === 0) {
                ...
                completedDeliveries.forEach(delivery => {
                    ...
                    // Format items received without truncation
...
function setupStaticExportButtons() {
    ...
    function exportResidentsToExcel() {
        try {
            ...
            const filename = `${barangayName}_Residents_${new Date().toISOString().split('T')[0]}.xlsx`;
